	...
	MobileCalendar.prototype = {
		...
		bindEvent: function(type) {
			...
			function popupDate(e) {
				...
				var time_min = _self.gearDate.querySelector(".time_mm");
				date_yy.addEventListener('touchstart', gearTouchStart);
				date_mm.addEventListener('touchstart', gearTouchStart);
				date_dd.addEventListener('touchstart', gearTouchStart);
				time_hh.addEventListener('touchstart', gearTouchStart);
                time_min.addEventListener('touchstart',gearTouchStart);
				date_yy.addEventListener('touchmove', gearTouchMove);
				date_mm.addEventListener('touchmove', gearTouchMove);
				date_dd.addEventListener('touchmove', gearTouchMove);
				time_hh.addEventListener('touchmove', gearTouchStart);
                time_min.addEventListener('touchmove', gearTouchStart);
				date_yy.addEventListener('touchend', gearTouchEnd);
				date_mm.addEventListener('touchend', gearTouchEnd);
				date_dd.addEventListener('touchend', gearTouchEnd);
				time_hh.addEventListener('touchend', gearTouchStart);
                time_min.addEventListener('touchend', gearTouchStart);
			}
			//初始化年月日插件默认值
			function dateCtrlInit() {
				...
				if (/^\d{4}-\d{1,2}-\d{1,2}-\d{1,2}$/.test(_self.trigger.value)) {
					rs = _self.trigger.value.match(/(^|-)\d{1,4}/g);
					dateArr.yy = rs[0] - _self.minY;
					dateArr.mm = rs[1].replace(/-/g, "") - 1;
					dateArr.dd = rs[2].replace(/-/g, "") - 1;
					dateArr.hh = rs[3].replace(/-/g, "") - 1;
				} else {
			...
			function popupDateTime(e) {
				...
				dateTimeCtrlInit();
				var lcalendar_cancel = _self.gearDate.querySelector(".lcalendar_cancel");
                _self.gearDate.addEventListener('touchstart',closeMobileCalendar);
                _self.gearDate.querySelector(".slideInUp").addEventListener('touchstart',function (e) {e.stopPropagation()});
				lcalendar_cancel.addEventListener('touchstart', closeMobileCalendar);
				var lcalendar_finish = _self.gearDate.querySelector(".lcalendar_finish");
			...
			function dateTimeCtrlInit() {
				...
				if (/^\d{4}-\d{1,2}-\d{1,2}-\d{1,2}:\d{2}$/.test(_self.trigger.value)) {
...
//					dateArr.hh = parseInt(rs[3].replace(/\s0?/g, ""));
			...
			function setDateGearTooth() {
				...
				if (time_hh && time_hh.getAttribute("val")) {
					...
					var minH = 0;
					//当年份到达最大值
					if (yyVal == passY - 1 && _self.maxM == mmVal + 1 && _self.maxD == ddVal+1 ) {
						maxH = _self.maxH - 1;
					}
					//当年份到达最小值
					if (yyVal == 0 && _self.minM == mmVal + 1 && _self.minD == ddVal + 1 ) {
						minH = _self.minH - 1;
					}
					var date_mm = _self.gearDate.querySelector(".date_mm");
					var mmVal = ~~date_mm.getAttribute('val');
					var date_dd = _self.gearDate.querySelector(".date_dd");
					var ddVal = ~~date_dd.getAttribute('val');
                    minH = start_hh
                    maxH = end_hh;
					// //当天
					var M = (now.getMonth() + 1 ).toString();
					var H = now.getDate().toString();
					var M1 = (now.getMonth() + 2 ).toString();//往后一个月
					var M2 = (now.getMonth() + 3 ).toString();//往后两个月
					var mm = (mmVal + 1).toString() + (ddVal + 1).toString();
					...
					//p 当前节点前后需要展示的节点个数
					for (var p = 0; p < maxH - minH + 1; p++) {
						itemStr += "<div class='tooth'>" + (minH + p) + "</div>";
					}

					time_hh.innerHTML = itemStr;
			...
			function setTimeGearTooth() {
				...
				var date_yy = _self.gearDate.querySelector(".date_yy");
				var yyVal = ~~date_yy.getAttribute('val');
				var date_mm = _self.gearDate.querySelector(".date_mm");
				var mmVal = ~~date_mm.getAttribute('val');
                var date_dd = _self.gearDate.querySelector(".date_dd");
                var ddVal = ~~date_dd.getAttribute('val');
                ...
                if(Date.parse(new Date(firstTime))<Date.parse(new Date())){
                	//开始的小时，分钟设置为特定分钟
                	if(mmVal != now.getMonth()){
					   console.log(ddVal);

					   if(ddVal == (now.getDate()-1)){
							 minMin = start_min;
							console.log(minMin);
						  /*if(hhVal == start_hh ){
							 console.log("1");
					...
					if(now.getDate()==(ddVal+1)){
						if(hhVal == now.getHours()){
							console.log("4");
							minMin = now.getMinutes();
						}
					}
                }
                //可选日期未到
				...
				if (time_mm && time_mm.getAttribute("val")) {
					...
					for (var g = 0; g <= range; g++) {
						i += "<div class='tooth'>" + (minMin + g) + "</div>";
					}
					time_mm.innerHTML = i;
                    ...
                    } else if (mmVal < minMin) {
                        mmVal = minMin;
                        time_mm.setAttribute("val", mmVal);
                    }

					time_mm.style["-webkit-transform"] = 'translate3d(0,' + (8 - (mmVal - minMin) * 2) + 'em,0)';
					time_mm.setAttribute('top', 8 - (mmVal - minMin) * 2 + 'em');
				} else {
					return
				}
				...
				target["int_" + target.id] = setInterval(function() {
					...
					switch (target.dataset.datetype) {
						...
						case "time_hh":
							...
							if (stopGear) {
								var gearVal = Math.abs(pos - 8) / 2 + minH;
								setGear(target, gearVal);
								clearInterval(target["int_" + target.id]);
							}
							break;
						...
						case "time_mm":
                            ...
                            var minTop = 8 - (maxMin - minMin) * 2;
                            if (pos < minTop) {
                                pos = minTop;
                                stopGear = true;
                            }
                            if (stopGear) {
                                var gearVal = Math.abs(pos - 8) / 2 + minMin;
                                setGear(target, gearVal);
                                clearInterval(target["int_" + target.id]);
                            }
							// if (pos < -110) {
